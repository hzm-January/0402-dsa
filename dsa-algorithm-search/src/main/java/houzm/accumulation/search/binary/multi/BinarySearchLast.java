...
public class BinarySearchLast {
    public static void main(String[] args) {
        ...
        //1. 查找最后一个目标元素
        System.out.println(0 + "在数组中的最后一次出现的索引为：" + binarySearchLast(arr, 0));
        ...
        System.out.println(10 + "在数组中的最后一次出现的索引为：" + binarySearchLast(arr, 10));
    }
    ...
    private static int binarySearchLast(int[] arr, int value, int low, int high) {
        while (low <= high) {
            ...
            if (arr[middle] == value) {
                ...
                /*if (middle == 0) {
                    //如果目标元素<数组中最小值，那么目标元素在目标数组中不存在返回-1
                    return -1;
                }
